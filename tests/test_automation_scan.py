...
class AutomationScanTests(unittest.TestCase):
    ...
    def test_run_scan_blocks_exhausted_recovered_imported_bundle_after_corruption(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inbox = root / "20_Raw/inbox"
            inbox.mkdir(parents=True)
            source_path = inbox / "exhausted-recovered-bundle.txt"
            source_path.write_text(
                "# Market Positioning\n\nBusiness strategy and positioning for the market moat.\n",
                encoding="utf-8",
            )
            state_path = root / "automation-state.json"
            bundle_path = _dated_bundle_path(root, "exhausted-recovered-bundle")
            content_hash = hashlib.sha256(source_path.read_bytes()).hexdigest()
            _write_imported_bundle_metadata(bundle_path, source_path, content_hash, retry_count=2)
            source_path.unlink()
            state_path.write_text("{not json", encoding="utf-8")
            ...
            final_state = load_source_state(state_path)
            source_key = "local-file:20_Raw/inbox/exhausted-recovered-bundle.txt"
            ...
            self.assertEqual(final_state["failed_items"], [])
            self.assertEqual(final_state["exhausted_failed_items"], [])
            self.assertEqual(final_state["last_scan"]["blocked_exhausted_count"], 1)
            self.assertIn(source_key, final_state["processed_sources"])
            self.assertEqual(final_state["processed_sources"][source_key]["stage"], "imported")
            self.assertEqual(final_state["processed_sources"][source_key]["retry_count"], 2)

    def test_run_scan_blocks_exhausted_recovered_imported_bundle_when_source_still_exists(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inbox = root / "20_Raw/inbox"
            inbox.mkdir(parents=True)
            source_path = inbox / "exhausted-source-present.txt"
            source_path.write_text(
                "# Market Positioning\n\nBusiness strategy and positioning for the market moat.\n",
                encoding="utf-8",
            )
            state_path = root / "automation-state.json"
            bundle_path = _dated_bundle_path(root, "exhausted-source-present")
            content_hash = hashlib.sha256(source_path.read_bytes()).hexdigest()
            _write_imported_bundle_metadata(bundle_path, source_path, content_hash, retry_count=2)
            state_path.write_text("{not json", encoding="utf-8")
            ...
            final_state = load_source_state(state_path)
            source_key = "local-file:20_Raw/inbox/exhausted-source-present.txt"
    ...
    def test_run_scan_recovers_imported_bundle_after_metadata_write_failure_and_source_removal_preserves_retry_count(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            inbox = root / "20_Raw/inbox"
            inbox.mkdir(parents=True)
            note_path = inbox / "preserve-retry-count.txt"
            note_path.write_text(
                "# Market Positioning\n\nBusiness strategy and positioning for the market moat.\n",
                encoding="utf-8",
            )
            ...
            final_state = load_source_state(state_path)
            source_key = "local-file:20_Raw/inbox/preserve-retry-count.txt"
            ...
            self.assertEqual(compile_mock.call_count, 1)
            self.assertEqual(final_state["processed_sources"][source_key]["stage"], "compiled")
            self.assertEqual(final_state["processed_sources"][source_key]["retry_count"], 1)

    def test_run_scan_recovers_imported_bundle_after_metadata_write_failure_and_second_compile_failure_exhausts_retry_budget(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inbox = root / "20_Raw/inbox"
            inbox.mkdir(parents=True)
            note_path = inbox / "exhaust-after-corruption.txt"
            note_path.write_text(
                "# Market Positioning\n\nBusiness strategy and positioning for the market moat.\n",
                encoding="utf-8",
            )
            ...
            final_state = load_source_state(state_path)
            source_key = "local-file:20_Raw/inbox/exhaust-after-corruption.txt"
